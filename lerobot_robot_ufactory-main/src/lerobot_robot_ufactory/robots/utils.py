...
# 工厂函数。看到 YAML 里的 robot.type 后决定实例化哪个 robot。
...
def make_robot_from_config(config: RobotConfig) -> Robot:
    ...
    elif config.type == "uf::piper":
        from lerobot_robot_ufactory_piper import PiperFollower
        return PiperFollower(config)
    elif config.type == "uf::dual_piper":
        from lerobot_robot_ufactory_piper import DualPiperFollower
        return DualPiperFollower(config)
    else:
